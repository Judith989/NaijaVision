...
type Submission = { id: string; status: string; expected_recordings: number; accepted_recordings: number; compensation_amount: number | null; compensation_currency: string | null; created_at: string; submitted_at: string | null; reviewed_at: string | null; paid_at: string | null };
...
export default function DashboardPage() {
  ...
  const [staffRequestStatus, setStaffRequestStatus] = useState("none");
  const [staffRequestMessage, setStaffRequestMessage] = useState("");
  ...
  const [policy, setPolicy] = useState<{ amount: number; currency: string; minimum_accepted_recordings: number } | null>(null);
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      const [profileResult, submissionsResult, surveyResult, consentResult, noticeResult, payoutResult, policyResult, paymentResult, recordingResult] = await Promise.all([
        supabase.from("profiles").select("participant_id,staff_request_status").eq("user_id", data.user.id).maybeSingle(),
        supabase.from("submissions").select("id,status,expected_recordings,accepted_recordings,compensation_amount,compensation_currency,created_at,submitted_at,reviewed_at,paid_at").eq("user_id", data.user.id).order("created_at", { ascending: false }),
        ...
        supabase.from("compensation_policies").select("amount,currency,minimum_accepted_recordings").is("retired_at", null).order("effective_at", { ascending: false }).limit(1).maybeSingle(),
      ...
      setStaffRequestStatus(profileResult.data?.staff_request_status || "none");
  ...
  async function requestStaffAccess() {
    if (!window.confirm("Request internal staff access? An administrator must review and approve the request.")) return;
    const { error } = await getSupabase()!.rpc("request_staff_access");
    if (error) setStaffRequestMessage(error.message);
    else {
      setStaffRequestStatus("pending");
      setStaffRequestMessage("Staff access request submitted.");
    }
  }
  ...
  const nextLabel = requiresRedo ? "Fix requested recordings" : latest && ["recording", "uploading", "draft"].includes(latest.status) ? "Continue contribution" : latest && ["submitted", "automated_qc", "awaiting_review", "resubmitted"].includes(latest.status) ? "View review status" : "Begin contribution";
  ...
  return <main><header className="topbar"><Link href="/" className="brand"><span className="brand-icon"><i /><i /><i /></span><span>Naija<span>Vision</span></span></Link><div className="top-context"><span className="privacy-dot" /> Participant dashboard</div><button className="admin-link" onClick={signOut}>Sign out</button></header>
    <section className="dashboard-shell enhanced-dashboard">
      ...
      <div className="dashboard-primary-grid"><section className="dashboard-card progress-card"><div className="card-heading"><div><small>Contribution progress</small><h2>{progress}% complete</h2></div><span>{completed}/{target} recordings</span></div><div className="dashboard-progress"><i style={{ width: `${progress}%` }} /></div><div className="progress-steps">{progressSteps.map(([label, done], index) => <div className={done ? "done" : index === progressSteps.findIndex((item) => !item[1]) ? "current" : ""} key={label}><span>{done ? "✓" : index + 1}</span><b>{label}</b></div>)}</div></section><aside className="dashboard-card compensation-card"><small>Compensation</small><h2>{policy ? `${policy.currency === "NGN" ? "₦" : policy.currency + " "}${policy.amount.toLocaleString()}` : "Policy pending"}</h2><span className={`eligibility ${latestPayment?.status === "paid" ? "eligible" : ""}`}>{latestPayment ? humanStatus(latestPayment.status) : "Not yet eligible"}</span><div className="payment-destination"><small>Payment destination</small><b>{payout ? `${payout.bank_name} ••••${payout.account_last4}` : "Add and verify bank details"}</b></div><p>Compensation becomes payable only after the complete submission passes human review.</p></aside></div>
      ...
      <section className="dashboard-card" id="notifications"><div className="card-heading"><div><small>Account updates</small><h2>Notifications</h2></div><span>{notices.filter((item) => !item.read_at).length} unread</span></div>{notices.length ? <div className="dashboard-notices">{notices.map((notice) => <div className={!notice.read_at ? "unread" : ""} key={notice.id}><span>i</span><div><b>{notice.title}</b><p>{notice.message}</p><small>{new Date(notice.created_at).toLocaleDateString()}</small></div></div>)}</div> : <div className="dashboard-empty compact"><p>No updates yet. Review decisions and payment changes will appear here.</p></div>}</section>
