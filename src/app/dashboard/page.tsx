...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data }) => {
      ...
      const [profileResult, submissionsResult, surveyResult, consentResult, noticeResult, payoutResult, policyResult, paymentResult, recordingResult, reviewerPaymentResult] = await Promise.all([
        ...
        supabase.from("submissions").select("id,status,expected_recordings,accepted_recordings,compensation_amount,compensation_currency,compensation_rate,compensation_basis,completed_language_count,created_at,submitted_at,reviewed_at,paid_at").eq("user_id", data.user.id).order("created_at", { ascending: false }),
  ...
  const completedLanguageCount = latest?.completed_language_count ?? 0;
