    ...
    <main>
      ...
      {step === "reviewer" && (
        <section className="admin-shell">
          ...
          {currentRole === "reviewer" ? <div className="review-decision recommendation-panel"><div><h3>Reviewer recommendation</h3><p>{everyRecordingReviewed ? "All recordings have a decision. Request redo for problem clips or recommend approval when every clip passes." : `Review every recording first. ${reviewerRecords.filter((record) => !record.review_decision).length} still need a decision.`}</p><textarea value={reviewComments} onChange={(event) => setReviewComments(event.target.value)} placeholder="Give clear instructions for clips that must be recorded again" /></div><button className="secondary" disabled={!hasRecordingForRedo} title={!hasRecordingForRedo ? "Mark at least one recording for redo first" : ""} onClick={() => submitReviewRecommendation("changes_requested")}>Return marked clips</button><button className="primary" disabled={!everyRecordingApproved} title={!everyRecordingApproved ? "Every recording must be approved first" : ""} onClick={() => submitReviewRecommendation("approved")}>Recommend approval</button></div> : <div className="review-decision"><div><h3>Administrator final decision</h3><p>{reviewRecommendation ? `Reviewer recommendation: ${reviewRecommendation.recommendation.replaceAll("_", " ")}. ${reviewRecommendation.comments || "No reviewer comments."}` : "A reviewer recommendation is required for approval. Full rejection is reserved for serious account-level or consent issues."}</p><textarea value={adminDecisionComments} onChange={(event) => setAdminDecisionComments(event.target.value)} placeholder="Required only for full rejection. Explain the account-level, consent, fraud, or eligibility reason." /></div><button className="secondary" disabled={!hasRecordingForRedo} onClick={() => { if (window.confirm(`Return only the marked clips from ${selectedReviewSubmission?.participant_id} for redo?`)) makeReviewDecision("changes_requested", reviewRecommendation?.comments || adminDecisionComments); }}>Return marked clips</button><button className="secondary danger-border" disabled={adminDecisionComments.trim().length < 10} title={adminDecisionComments.trim().length < 10 ? "Enter an account-level reason of at least 10 characters" : ""} onClick={() => { if (window.confirm(`Reject the entire submission from ${selectedReviewSubmission?.participant_id}? Use this only for an account-level, consent, fraud, or eligibility issue.`)) makeReviewDecision("rejected", adminDecisionComments); }}>Reject entire submission</button><button className="primary" disabled={reviewRecommendation?.recommendation !== "approved"} title={reviewRecommendation?.recommendation !== "approved" ? "A reviewer approval recommendation is required" : ""} onClick={() => { if (window.confirm(`Give final approval to submission ${selectedReviewId.slice(0, 8)} from ${selectedReviewSubmission?.participant_id}?`)) makeReviewDecision("approved", adminDecisionComments); }}>Final approval</button><button className="primary payment" disabled={selectedReviewSubmission?.status !== "payment_eligible"} onClick={approvePayment}>Process payment</button></div>}
