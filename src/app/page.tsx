    ...
    <main>
      ...
      {step === "reviewer" && (
        <section className="admin-shell">
          ...
          {currentRole === "reviewer" ? <div className="review-decision recommendation-panel"><div><h3>Reviewer recommendation</h3><p>{everyRecordingReviewed ? "All recordings have a decision. You can now send a recommendation." : `Review every recording first. ${reviewerRecords.filter((record) => !record.review_decision).length} still need a decision.`}</p><textarea value={reviewComments} onChange={(event) => setReviewComments(event.target.value)} placeholder="Explain quality problems, redo instructions, or reasons for your recommendation" /></div><button className="secondary" disabled={!hasRecordingForRedo} title={!hasRecordingForRedo ? "Mark at least one recording for redo or decline first" : ""} onClick={() => submitReviewRecommendation("changes_requested")}>Recommend redo</button><button className="secondary danger-border" disabled={!everyRecordingReviewed} title={!everyRecordingReviewed ? "Review every recording first" : ""} onClick={() => { if (window.confirm(`Recommend declining submission ${selectedReviewId.slice(0, 8)} from ${selectedReviewSubmission?.participant_id}?`)) submitReviewRecommendation("rejected"); }}>Recommend decline</button><button className="primary" disabled={!everyRecordingApproved} title={!everyRecordingApproved ? "Every recording must be approved first" : ""} onClick={() => submitReviewRecommendation("approved")}>Recommend approval</button></div> : <div className="review-decision"><div><h3>Administrator final decision</h3><p>{reviewRecommendation ? `Reviewer recommendation: ${reviewRecommendation.recommendation.replaceAll("_", " ")}. ${reviewRecommendation.comments || "No reviewer comments."}` : "A reviewer recommendation is required before the final decision."}</p></div><button className="secondary" disabled={!reviewRecommendation} onClick={() => { if (window.confirm(`Return submission ${selectedReviewId.slice(0, 8)} from ${selectedReviewSubmission?.participant_id} for redo?`)) makeReviewDecision("changes_requested"); }}>Return for redo</button><button className="secondary danger-border" disabled={!reviewRecommendation} onClick={() => { if (window.confirm(`Decline submission ${selectedReviewId.slice(0, 8)} from ${selectedReviewSubmission?.participant_id}? This makes it ineligible for payment.`)) makeReviewDecision("rejected"); }}>Decline submission</button><button className="primary" disabled={reviewRecommendation?.recommendation !== "approved"} title={reviewRecommendation?.recommendation !== "approved" ? "A reviewer approval recommendation is required" : ""} onClick={() => { if (window.confirm(`Give final approval to submission ${selectedReviewId.slice(0, 8)} from ${selectedReviewSubmission?.participant_id}?`)) makeReviewDecision("approved"); }}>Final approval</button><button className="primary payment" disabled={selectedReviewSubmission?.status !== "payment_eligible"} onClick={approvePayment}>Process payment</button></div>}
